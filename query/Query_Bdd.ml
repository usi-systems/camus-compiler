...
let bdd_from_rules ?slices:(slices=40) (qs:QuerySpec.t) rules =
  ...
  let merge_rule m (qf, a) =
      ...
      QueryBdd.merge_nodes bdd m (node_for_rule (qf, a))
  in
  let bdd_for_slice rules_slice =
    List.fold_left ~init:bdd.empty_leaf ~f:merge_rule rules_slice
  in
  let slice_bdds =
    Parmap.parmap ~ncores:12 bdd_for_slice (Parmap.L rules_slices)
  in
  let fold_op = QueryBdd.merge_nodes bdd in
  bdd.root <- Parmap.parfold ~ncores:12 fold_op (Parmap.L slice_bdds) bdd.empty_leaf  fold_op;
